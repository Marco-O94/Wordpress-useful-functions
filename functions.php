...
* - WP QUERY
* - ACF QUERY
* - ACF QUERY (Import only products chosen in backend artisan or owner page)
...
/* ======== ACF REPEATER ======== */
/* Custom products chosen in backend panel */


function custom_products_block() {
	$posts = get_field("prodotti"); //Custom field has to be Post Object
	if($posts) {
	
foreach($posts as $post) {
	$post_title = get_the_title($post);
	$post_thumb = get_the_post_thumbnail($post);

$products = '<h1>' . $post_title . '</h1><img src="' . $post_thumb .  '" />';
}
}
}
add_shortcode('products_tab', 'custom_products_block');
